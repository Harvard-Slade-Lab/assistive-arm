...
def get_model_refactor(subject_name, model_path, params):

    osim_model, subject_name = getMuscleDrivenModel_refactor(subject_name = subject_name, 
                                                            model_path = model_path, 
                                                            params = params)
    if params["osim_GRF"] and 'sit' in params["motion"].lower():
        generate_model_with_contacts(model_name=subject_name, model=osim_model, params=params, contact_side='all')
    elif params["osim_GRF"] and 'stair' in params["motion"].lower():
        generate_model_with_contacts_stair(model_name=subject_name, model=osim_model, params=params, contact_side='all')
    elif params["osim_GRF"] and 'incline' in params["motion"].lower():
        generate_model_with_contacts_incline(model_name=subject_name, model=osim_model, params=params, contact_side='all')
...
def set_state_tracking_weights(moco_study: osim.MocoStudy, params):
    ...
    stateTrackingGoal.setWeightForState('/jointset/ground_pelvis/pelvis_tilt/value', 10)
    stateTrackingGoal.setWeightForState('/jointset/ground_pelvis/pelvis_tilt/speed', 10)
    stateTrackingGoal.setWeightForState('/jointset/back/lumbar_extension/value', 10)
    stateTrackingGoal.setWeightForState('/jointset/back/lumbar_extension/speed', 10)    
...
def set_reserve_actuator_control_bound(model):
    ...
    for i in range(forceSet.getSize()):
        ...
        # Set control bounds for reserve actuators between -1 and 1
        if "reserve_jointset_" in str(forcePath):
            this_coordinate_actuator = osim.CoordinateActuator.safeDownCast(this_actuator)
            this_coordinate_actuator.setMinControl(-1)
            this_coordinate_actuator.setMaxControl(1)                    

def set_moco_problem_weights(model, moco_study, params):
...
def set_moco_problem_solver_settings(moco_study: osim.MocoStudy, params, nb_max_iterations: int = None):
    ...
    solver.set_optim_convergence_tolerance(params['convergence_tolerance']) # Loosen the convergence tolerances, cf. doc
    solver.set_optim_constraint_tolerance(params['constraint_tolerance']) # Loosen the constraint tolerances, cf. doc

# def add_assistive_force_thigh(
#     coordName: str,
#     model: osim.Model,
#     name: str,
#     direction: osim.Vec3,
...
def add_assistive_force(
    ...
    if coordName == "pelvis_tx":
        ...
        assistActuator.setMinControl(-np.inf) #manage the bounds on ScalarActuator's control
        assistActuator.setMaxControl(np.inf)  #manage the bounds on ScalarActuator's control
        # assistActuator.setMinControl(-1) #manage the bounds on ScalarActuator's control
        # assistActuator.setMaxControl(1)  #manage the bounds on ScalarActuator's control
    if coordName == "pelvis_ty":
        print("control bounds assistive force y: [0,inf]")
        assistActuator.setMinControl(-np.inf) #manage the bounds on ScalarActuator's control
        assistActuator.setMaxControl(np.inf) #manage the bounds on ScalarActuator's control
