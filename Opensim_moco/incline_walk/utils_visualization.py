...
def plot_GRF_osim_v2(df_sim_floor, body_weight, fontsize, figsize, fig_path):
    """Plot ground reaction forces (GRFs) on the floor during simulation, normalized to body weight."""
    ...
    # Plot GRF components normalized by body weight
    ax.plot(df_sim_floor.time_perc, df_sim_floor['ground_force_r_vx'] / body_weight,
            linewidth=2, color=colors_floor['x'], label='GRF x')
    ax.plot(df_sim_floor.time_perc, df_sim_floor['ground_force_r_vy'] / body_weight,
            linewidth=2, color=colors_floor['y'], label='GRF y')
    ax.plot(df_sim_floor.time_perc, df_sim_floor['ground_force_r_vz'] / body_weight,
            linewidth=2, color=colors_floor['z'], label='GRF z')

    # Axis labels and styling
    ax.set_xlabel("Task completion [%]", fontsize=fontsize)
    ax.set_ylabel("Force [BW]", fontsize=fontsize)
    ax.set_ylim([-0.5, 1.5])
    ax.set_title("Ground Forces of right foot (Floor)", fontsize=fontsize)
    ax.legend(frameon=False, loc='center left', bbox_to_anchor=(1, 0.5))
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.tick_params(labelsize=fontsize)

    # Print max GRF value (vertical direction)
    max_vgrf = np.max(df_sim_floor['ground_force_r_vy']) / body_weight
    print(f"Maximum vertical GRF (floor): {max_vgrf:.2f} BW")

    # Save figure
    fig_title = 'sim_ground_reaction_force_torque.png'
...
def plot_muscle_activation_grid(df_sim, ref_m_activation, fontsize, figsize, fig_path):
    ...
    for i, (muscle_abbr, muscle_fullname) in enumerate(muscles):
        left = f'/forceset/{muscle_abbr}_r'
        ...
        if left in df_sim and right in df_sim:
            ...
            ax.plot(time, avg, label='right side only', color=color, linewidth=2)
...
def plot_residual_force_torque_others(df_sim, sim_params, grf_info, fontsize, figsize, fig_path):
    ...
    if grf_info is not None:
        ...
        residual_force_threshold = 0
        residual_torque_threshold = 0
    ...
    axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_hip_l_hip_adduction_l']*sim_params['reserve_actuator_opt_value'], label='hip_adduction_l')
    axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_hip_l_hip_rotation_l']*sim_params['reserve_actuator_opt_value'], label='hip_rotation_l')
    axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_walker_knee_l_knee_angle_l']*sim_params['reserve_actuator_opt_value'], label='knee_angle_l')
    axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ankle_l_ankle_angle_l']*sim_params['reserve_actuator_opt_value'], label='ankle_angle_l')
    axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_subtalar_l_subtalar_angle_l']*sim_params['reserve_actuator_opt_value'], label='subtalar_angle_l')
    axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_mtp_l_mtp_angle_l']*sim_params['reserve_actuator_opt_value'], label='mtp_angle_l')
    ...
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_hip_r_hip_adduction_r']*sim_params['reserve_actuator_opt_value'], label='hip_adduction_r')
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_hip_r_hip_rotation_r']*sim_params['reserve_actuator_opt_value'], label='hip_rotation_r')
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_walker_knee_r_knee_angle_r']*sim_params['reserve_actuator_opt_value'], label='knee_angle_r')
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ankle_r_ankle_angle_r']*sim_params['reserve_actuator_opt_value'], label='ankle_angle_r')
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_subtalar_r_subtalar_angle_r']*sim_params['reserve_actuator_opt_value'], label='subtalar_angle_r')
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_mtp_r_mtp_angle_r']*sim_params['reserve_actuator_opt_value'], label='mtp_angle_r')
...
def plot_residual_force_torque_pelvis(df_sim, sim_params, grf_info, fontsize, figsize, fig_path):
    ...
    if sim_params['assist_with_reserve_pelvis_txy']:
        axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_tx']*sim_params['reserve_pelvis_tx_ty_opt_value'], linewidth = 3, label='Pelvis x')
        axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_ty']*sim_params['reserve_pelvis_tx_ty_opt_value'], linewidth = 3, label='Pelvis y')    
    else:
        axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_tx']*sim_params['reserve_actuator_opt_value'], linewidth = 3, label='Pelvis x')
        axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_ty']*sim_params['reserve_actuator_opt_value'], linewidth = 3, label='Pelvis y')
       
    axes[0].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_tz']*sim_params['reserve_actuator_opt_value'], linewidth = 3, label='Pelvis z')
    ...
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_tilt']*sim_params['reserve_actuator_opt_value'], linewidth = 3, label='Pelvis tilt')
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_list']*sim_params['reserve_actuator_opt_value'], linewidth = 3, label='Pelvis list')
    axes[1].plot(df_sim['time_perc'], df_sim['/forceset/reserve_jointset_ground_pelvis_pelvis_rotation']*sim_params['reserve_actuator_opt_value'], linewidth = 3, label='Pelvis rotation')
    axes[1].axhline(y=residual_torque_threshold, linestyle='--', color='k')
    axes[1].axhline(y=-residual_torque_threshold, label='Residual torque\nthreshold', linestyle='--', color='k')
