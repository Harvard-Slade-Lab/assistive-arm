...
def compare_muscle_activation_grid(df_unassisted, df_assisted, fig_title, fontsize, figsize, fig_path):
    time = df_unassisted['time_perc']  # % of task completion
    ...
    for i, (muscle_abbr, muscle_fullname) in enumerate(muscles):
        ...
        # Unassisted
        if left in df_unassisted and right in df_unassisted:
            l_un = df_unassisted[left] * 100
            r_un = df_unassisted[right] * 100
            avg_un = (l_un + r_un) / 2
            ax.plot(time, avg_un, label='Unassisted', color=color, linewidth=2, alpha=1.0)
            mean_un = np.mean(avg_un)
            peak_un = np.max(avg_un)
            stats_text += f"Unasst: μ={mean_un:.1f}%, pk={peak_un:.1f}%\n"
        else:
            stats_text += f"Unasst: N/A\n"

        # Assisted
        if left in df_assisted and right in df_assisted:
            l_as = df_assisted[left] * 100
            r_as = df_assisted[right] * 100
            avg_as = (l_as + r_as) / 2
            ax.plot(time, avg_as, label='Assisted', color=color, linewidth=2, alpha=0.5)
            mean_as = np.mean(avg_as)
            peak_as = np.max(avg_as)
            stats_text += f"Asst: μ={mean_as:.1f}%, pk={peak_as:.1f}%"
        else:
            stats_text += f"Asst: N/A"

        # Add stats as annotation above panel
