...
# class DataKernel():
#     def __init__(self, trigno_base, host='10.250.135.59', port=65432):
...
class DataKernel():
    ...
    def GetData(self):
        """Check if data is ready from DelsysAPI via Aero CheckDataQueue() - Return True if data is ready
        Get data (PollData)
        Organize output channels by their GUID keys and map them to the correct indices
        Return array of all channel data
        """
        # Access AeroPy methods via self.TrigBase.TrigBase
        dataReady = self.TrigBase.TrigBase.CheckDataQueue()
        if dataReady:
            DataOut = self.TrigBase.TrigBase.PollData()
            outArr = [[] for _ in self.TrigBase.channel_guids]  # Use self.TrigBase to access channel_guids
            ...
            for guid in DataOut.Keys:
                ...
                if index is not None:
                    ...
                    # Handle unexpected GUIDs if necessary
                    pass
