...
def plot_residual_forces_torques_all(df: pd.DataFrame, config_file: dict, filter_key_infos: bool, alpha=None):
    ...
    # Add a main title
    fig_title = f"Residual Forces and Torques ({'assisted' if config_file['assistive_force'] else 'unassisted'})"
    fig.suptitle(fig_title, fontsize=22, y=1.02)

    plt.tight_layout()

    # if save:
    save_path = f'./result_figure/{fig_title}'
...
def plot_residual_forces_torques_all2(df: pd.DataFrame, config_file: dict, save:bool, title:str, filter_key_infos: bool, alpha=None):
    # Create a figure with a 2x2 grid, and split the lower right cell into two
    fig = plt.figure(figsize=(16, 8))
    gs = gridspec.GridSpec(2, 2, height_ratios=[1, 1], width_ratios=[1, 1])
    ...
    tilt = 'reserve_pelvis_tilt' if config_file["minimal_actuators"] else '/forceset/reserve_jointset_ground_pelvis_pelvis_tilt'
    list = 'reserve_pelvis_list' if config_file["minimal_actuators"] else '/forceset/reserve_jointset_ground_pelvis_pelvis_list'
    rotation = 'reserve_pelvis_rotation' if config_file["minimal_actuators"] else '/forceset/reserve_jointset_ground_pelvis_pelvis_rotation'

    if not config_file["minimal_actuators"]:
        hip_flexion_r = '/forceset/reserve_jointset_hip_r_hip_flexion_r'
        hip_adduction_r = '/forceset/reserve_jointset_hip_r_hip_adduction_r'
        hip_rotation_r  = '/forceset/reserve_jointset_hip_r_hip_rotation_r'
        knee_angle_r = '/forceset/reserve_jointset_walker_knee_r_knee_angle_r'
    ...
    if config_file["assist_with_reserve_pelvis_txy"]:
        ...
        if filter_key_infos == False:
            ax1.plot(df.percentage, df[tx]*config_file["reserve_pelvis_opt_value"], label=r"$F_x$", color=fx_color, linewidth=2.5)
            ax1.plot(df.percentage, df[ty]*config_file["reserve_pelvis_opt_value"], label=r"$F_y$", color=fy_color, linewidth=2.5)
            ax1.plot(df.percentage, df[tz]*config_file["actuator_magnitude"], label='Pelvis z', color='C2', linewidth=2.5)
        else:
            ax1.plot(df.percentage, df[tx]*config_file["reserve_pelvis_opt_value"], label=r"$F_x$", color=fx_color, linewidth=2.5)
            ax1.plot(df.percentage, df[ty]*config_file["reserve_pelvis_opt_value"], label=r"$F_y$", color=fy_color, linewidth=2.5)
            ax1.plot(df.percentage, df[tz]*config_file["actuator_magnitude"], label='Pelvis z', color='C2', alpha=alpha, linewidth=1)
    else:
        print("actuator_magnitude pelvis x/y/z", config_file["actuator_magnitude"])
        if filter_key_infos == False:
            ax1.plot(df.percentage, df[tx]*config_file["actuator_magnitude"], label='Pelvis x', linewidth=2.5)
            ax1.plot(df.percentage, df[ty]*config_file["actuator_magnitude"], label='Pelvis y', linewidth=2.5)
            ax1.plot(df.percentage, df[tz]*config_file["actuator_magnitude"], label='Pelvis z', linewidth=2.5)
        else:
            ax1.plot(df.percentage, df[tx]*config_file["actuator_magnitude"], label='Pelvis x', linewidth=2.5)
            ax1.plot(df.percentage, df[ty]*config_file["actuator_magnitude"], label='Pelvis y', linewidth=2.5)
            ax1.plot(df.percentage, df[tz]*config_file["actuator_magnitude"], label='Pelvis z', alpha=alpha, linewidth=1)
    
    ax1.set_xlabel("Sit-to-Stand Phase (%)")
    ax1.set_ylabel("Force (N)")
    ax1.set_title(f"Pelvis Residual Forces", fontsize=14)
    ...
    print("actuator_magnitude pelvis list/rot/tilt", config_file["actuator_magnitude"])
    if filter_key_infos == False:
        ax2.plot(df.percentage, df[list]*config_file["actuator_magnitude"], label='Pelvis list', linewidth=2.5)
        ax2.plot(df.percentage, df[rotation]*config_file["actuator_magnitude"], label='Pelvis rotation', linewidth=2.5)
        ax2.plot(df.percentage, df[tilt]*config_file["actuator_magnitude"], label='Pelvis tilt', linewidth=2.5)
    else:
        ax2.plot(df.percentage, df[list]*config_file["actuator_magnitude"], label='Pelvis list', alpha=alpha, linewidth=1)
        ax2.plot(df.percentage, df[rotation]*config_file["actuator_magnitude"], label='Pelvis rotation', alpha=alpha, linewidth=1)
        ax2.plot(df.percentage, df[tilt]*config_file["actuator_magnitude"], label='Pelvis tilt', linewidth=2.5)
    ax2.set_xlabel("Sit-to-Stand Phase (%)")
    ax2.set_ylabel("Torque (Nm)")
    ax2.set_title(f"Pelvis Residual Torques", fontsize=14)
    ...
    print("actuator_magnitude legs", config_file["actuator_magnitude"])
    if filter_key_infos == False:
        ax3.plot(df.percentage, df[hip_flexion_l]*config_file["actuator_magnitude"], label=f'Hip Flexion', linewidth=2.5)
        ax3.plot(df.percentage, df[hip_adduction_l]*config_file["actuator_magnitude"], label=f'Hip Adduction', linewidth=2.5)
        ax3.plot(df.percentage, df[hip_rotation_l]*config_file["actuator_magnitude"], label=f'Hip Rotation', linewidth=2.5)
        ax3.plot(df.percentage, df[knee_angle_l]*config_file["actuator_magnitude"], label=f'Knee Flexion', linewidth=2.5)
        ax3.plot(df.percentage, df[ankle_angle_l]*config_file["actuator_magnitude"], label=f'Ankle Flexion', linewidth=2.5)
        ax3.plot(df.percentage, df[subtalar_angle_l]*config_file["actuator_magnitude"], label=f'Subtalar Angle', linewidth=2.5)
        ax3.plot(df.percentage, df[mtp_angle_l]*config_file["actuator_magnitude"], label=f'Mtp Angle', linewidth=2.5)
    else:
        ax3.plot(df.percentage, df[hip_flexion_l]*config_file["actuator_magnitude"], label=f'Hip Flexion', linewidth=2.5)
        ax3.plot(df.percentage, df[hip_adduction_l]*config_file["actuator_magnitude"], label=f'Hip Adduction', alpha=alpha, linewidth=1)
        ax3.plot(df.percentage, df[hip_rotation_l]*config_file["actuator_magnitude"], label=f'Hip Rotation', alpha=alpha, linewidth=1)
        ax3.plot(df.percentage, df[knee_angle_l]*config_file["actuator_magnitude"], label=f'Knee Flexion', linewidth=2.5)
        ax3.plot(df.percentage, df[ankle_angle_l]*config_file["actuator_magnitude"], label=f'Ankle Flexion', linewidth=2.5)
        ax3.plot(df.percentage, df[subtalar_angle_l]*config_file["actuator_magnitude"], label=f'Subtalar Angle', alpha=alpha, linewidth=1)
        ax3.plot(df.percentage, df[mtp_angle_l]*config_file["actuator_magnitude"], label=f'Mtp Angle', alpha=alpha, linewidth=1)
    ax3.set_xlabel("Sit-to-Stand Phase (%)")
    ax3.set_ylabel("Torque (Nm)")
    ax3.set_title(f"Left Leg Residual Torques", fontsize=14)
    ...
    ax4 = fig.add_subplot(gs[1, 1])
    if filter_key_infos == False:
        ax4.plot(df.percentage, df[hip_flexion_r]*config_file["actuator_magnitude"], label='Hip Flexion', linewidth=2.5)
        ax4.plot(df.percentage, df[hip_adduction_r]*config_file["actuator_magnitude"], label='Hip Adduction', linewidth=2.5)
        ax4.plot(df.percentage, df[hip_rotation_r]*config_file["actuator_magnitude"], label=f'Hip Rotation', linewidth=2.5)
        ax4.plot(df.percentage, df[knee_angle_r]*config_file["actuator_magnitude"], label=f'Knee Flexion', linewidth=2.5)
        ax4.plot(df.percentage, df[ankle_angle_r]*config_file["actuator_magnitude"], label=f'Ankle Flexion', linewidth=2.5)
        ax4.plot(df.percentage, df[subtalar_angle_r]*config_file["actuator_magnitude"], label=f'Subtalar Angle', linewidth=2.5)
        ax4.plot(df.percentage, df[mtp_angle_r]*config_file["actuator_magnitude"], label=f'Mtp Angle', linewidth=2.5)
    else:
        ax4.plot(df.percentage, df[hip_flexion_r]*config_file["actuator_magnitude"], label='Hip Flexion', linewidth=2.5)
        ax4.plot(df.percentage, df[hip_adduction_r]*config_file["actuator_magnitude"], label='Hip Adduction', alpha=alpha, linewidth=1)
        ax4.plot(df.percentage, df[hip_rotation_r]*config_file["actuator_magnitude"], label=f'Hip Rotation', alpha=alpha, linewidth=1)
        ax4.plot(df.percentage, df[knee_angle_r]*config_file["actuator_magnitude"], label=f'Knee Flexion', linewidth=2.5)
        ax4.plot(df.percentage, df[ankle_angle_r]*config_file["actuator_magnitude"], label=f'Ankle Flexion', linewidth=2.5)
        ax4.plot(df.percentage, df[subtalar_angle_r]*config_file["actuator_magnitude"], label=f'Subtalar Angle', alpha=alpha, linewidth=1)
        ax4.plot(df.percentage, df[mtp_angle_r]*config_file["actuator_magnitude"], label=f'Mtp Angle', alpha=alpha, linewidth=1)
    ax4.set_xlabel("Sit-to-Stand Phase (%)")
    ax4.set_ylabel("Torque (Nm)")
    ax4.set_title(f"Right Leg Residual Torques", fontsize=14)
    ax4.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=11)
    ...
    # Add a main title
    fig.suptitle(f"Simulated Residual Actuators ({'Assisted Case' if config_file['assistive_force'] else 'Unassisted Case'})", fontsize=16, y=1.02)
    if save:
        save_path = f'./result_figure/{title}'
        # Check if the directory exists. If not, create it.
        directory = os.path.dirname(save_path)
        if not os.path.exists(directory):
            os.makedirs(directory)
...
def format_muscle_name(name):
    # Replace 'angle' with 'flexion'
    name = name.replace('angle', 'flexion')
    # Split the name by underscore
    words = name.split('_')
    # Capitalize each word and join them without underscores
