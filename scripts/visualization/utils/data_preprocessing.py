...
def butterworth_low_pass(cutoff, fs, order):
    nyquist = 0.5 * fs
...
def butterworth_low_pass_df(df, cutoff, order):
    """
    Applies a Butterworth low-pass filter to each column in the DataFrame.

    Parameters:
    - df: Pandas DataFrame where each column is a signal to be filtered.
    ...
    Returns:
    - filtered_df: A DataFrame with the filtered signals.
    """
    
    fs =1/np.mean(np.diff(df['time']))
    b, a = butterworth_low_pass(cutoff, fs, order)
    filtered_df = df.apply(lambda col: apply_filter(col, b, a), axis=0)
    return filtered_df
