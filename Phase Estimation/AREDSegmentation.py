...
threshold_factor = 0.3     # Factor to scale down the threshold (30% of Otsu's threshold)
min_duration = 20          # Minimum duration of a motion segment in samples
...
class RefinedMotionSegmenter:
    ...
    def refine_motion_boundaries(self, ared_signal, rough_start, rough_end):
        ...
        # Refine the start point - search backward from rough_start
        refined_start = rough_start
        for i in range(rough_start, 0, -1):
            if ared_signal[i] < edge_threshold:
                refined_start = i + 1  # The point just after falling below threshold
                break
        
        # Refine the end point - search forward from rough_end
        refined_end = rough_end
        for i in range(rough_end, len(ared_signal)-1):
            if ared_signal[i] < edge_threshold:
                refined_end = i - 1  # The point just before falling below threshold
                break
        ...
        start_search_range = max(0, refined_start - 100), min(len(ared_diff_smoothed)-1, refined_start + 100)
        for i in range(start_search_range[0], start_search_range[1]):
            if ared_diff_smoothed[i] > ared_diff_smoothed[refined_start] * 0.2:
                refined_start = i
                break
        
        # For end point: look for significant negative rate of change
        end_search_range = max(0, refined_end - 100), min(len(ared_diff_smoothed)-1, refined_end + 100)
        for i in range(end_search_range[1], end_search_range[0], -1):
            if ared_diff_smoothed[i] < ared_diff_smoothed[refined_end] * 0.2:
                refined_end = i
                break
    ...
    def segment_motion(self, magnitude):
        ...
        diff = np.diff(np.concatenate(([0], filtered_mask, [0])))
        ...
        return refined_start, refined_end, ared_signal, threshold
    
    def plot_segmentation(self, magnitude, motion_start, motion_end, ared_signal=None, threshold=None, frequency=None):
        """
...
def AREDSegmentation(raw_magnitude, timestamp, plot_flag=False):
    ...
    # Segment motion
    motion_start, motion_end, ared_signal, threshold = segmenter.segment_motion(magnitude_np)
    ...
    if plot_flag:
        segmenter.plot_segmentation(magnitude_np, motion_start, motion_end, ared_signal, threshold, frequency)
